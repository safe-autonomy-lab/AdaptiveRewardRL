...
class CraftWorld:
    def __init__(self, file_map):
        self.file_map = os.getcwd() + file_map
    ...
    def step(self, action):
        ...
        state = np.array([(self.agent.i, self.agent.j)])
        # dummy reward
        reward = 0.
        # dummy done, done will be determined by LTL formula, and episode length, will be set in LTL grid env.py
        done = False
        info = {}
    ...
    def get_model(self):
        ...
        """
        S = [(x,y) for x in range(1, self.map_height-1) for y in range(1, self.map_width-1)] # States
    ...
    def _load_map(self, file_map):
        ...
        for l in f:
            ...
            for e in l.rstrip():
                ...
                if e == "A":   
                    self.agent = Agent(i, j, self.actions)
                row.append(entity)
                j += 1
        ...
        S, _, self.label, _ = self.get_model()
        self.shape = (self.map_height, self.map_width)