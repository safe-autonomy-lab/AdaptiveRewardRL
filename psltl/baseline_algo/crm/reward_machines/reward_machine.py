...
from psltl.baseline_algo.crm.reward_machines.reward_machine_utils import evaluate_dnf, value_iteration
import time
...
class RewardMachine:
    ...
    def _compute_next_state(self, u1, true_props):
        for u2 in self.delta_u[u1]:
            if evaluate_dnf(self.delta_u[u1][u2], true_props):
                return u2
        return self.fall_off # no transition is defined for true_props
    ...
    def step(self, u1, true_props, s_info, add_rs=False, env_done=False):
        ...
        done = (u2 == self.fall_off or u2 in self.G or u2 == self.terminal_u)
        # Getting the reward
