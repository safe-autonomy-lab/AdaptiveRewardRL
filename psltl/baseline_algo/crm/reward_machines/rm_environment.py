...
class RewardMachineEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.missing:
            # this will make the agent blind to the goal so that goal can never be achieved.
            # for taxi, office
            if "g" in true_props:
                true_props = true_props.replace("g", "")
            # for water
            if "c" in true_props and self.current_u_id == 3:
                true_props = true_props.replace("c", "")
            # for cheetah
            if "a" in true_props and self.current_u_id == 4:
                true_props = true_props.replace("a", "")
        ...
        info.update({"label": true_props})
        info.update({"rm_state": self.current_u_id})
...
class RewardMachineWrapper(gym.Wrapper):
    def __init__(self, env, add_crm, add_rs, gamma, rs_gamma, missing):
        """
        RM wrapper
        --------------------
