...
from psltl.baseline_algo.qrm.src.reward_machines.reward_functions import *
from psltl.baseline_algo.qrm.src.reward_machines.reward_machine_utils import evaluate_dnf, are_these_machines_equivalent, value_iteration
import time
...
class RewardMachine:
    def __init__(self, file, use_rs, gamma):
        ...
        self.G = set()    # set of success states 
    ...
    def get_next_state(self, u1, true_props):
        # if true_props != "" and true_props != "a" and true_props != "n":
            # print("=" * 10)
            # print("qrm_rog/ reward machine / reward machines. this is org reward machine!")
            # print("u1", u1)
            # print("label", true_props)
        if u1 < self.u_broken:
            for u2 in self.delta_u[u1]:
                predicate = evaluate_dnf(self.delta_u[u1][u2], true_props)
                if predicate:
                    return u2
